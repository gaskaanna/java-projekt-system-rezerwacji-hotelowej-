...
public class ReservationController {
    ...
    public ResponseEntity<Reservation> createReservation (
        ...
        // For USER role, ensure they can only create reservations for themselves
        if (userDetails != null && reservation.getUser() != null) {
            boolean isAdmin = userDetails.getAuthorities().stream()
                .anyMatch(a -> a.getAuthority().equals("ROLE_" + RoleName.ADMIN.name()));

            if (!isAdmin && !reservation.getUser().getEmail().equals(userDetails.getUsername())) {
